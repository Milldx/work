  ...
  loss_middle: {
    ...
    days: [
      { id: 1, name: "Круговая на всё тело", warmUp: ["5 мин лёгкий бег", "Суставная разминка"],
        main: [
          { title: "Круг 1 (3 подхода)", exercises: [
  ...
  loss_advanced: {
    ...
    days: [
      { id: 1, name: "HIIT + функционалка", warmUp: ["10 мин разминка", "Прыжки на скакалке"],
        main: [
          { title: "Взрывная тренировка", exercises: [
      ...
      { id: 2, name: "Силовая выносливость", warmUp: ["Велотренажёр 10 мин"],
        main: [
          { title: "Круговая на всё тело", exercises: [
  ...
  mass_middle: {
    ...
    days: [
      { id: 1, name: "Грудь + Плечи", warmUp: ["10 мин общая разминка", "Разогрев ротаторной манжеты"],
        main: [
          { title: "Основная часть", exercises: [
  ...
  keep_beginner: {
    ...
    days: [
      { id: 1, name: "Общая физическая подготовка", warmUp: ["Суставная гимнастика 5 мин"],
        main: [
          { title: "Круг для всего тела", exercises: [
  ...
  keep_advanced: {
    ...
    days: [
      { id: 1, name: "Силовая выносливость", warmUp: ["Динамическая растяжка"],
        main: [
          { title: "Круговая из базовых", exercises: [
...
// -------------------------- РАСШИРЕННОЕ ПИТАНИЕ (ЖЁСТКОЕ ПО КОМБИНАЦИЯМ) --------------------------
function generateNutritionAndTips(user) {
  const goal = user.goal
  const level = user.level
  const key = `${goal}_${level}`
  const plans = {
    loss_beginner: { main: "Дефицит 500 ккал. Упор на белок.", foods: ["Куриная грудка", "Творог 5%", "Гречка", "Овощи"], schedule: [{ name: "Завтрак", desc: "Омлет + овсянка" }, { name: "Обед", desc: "Курица + гречка" }, { name: "Ужин", desc: "Рыба + овощи" }], water: "2.5 л", note: "Не голодай." },
    loss_middle: { main: "Дефицит 500 ккал. Больше клетчатки.", foods: ["Индейка", "Рис", "Овощи", "Яйца"], schedule: [{ name: "Завтрак", desc: "Каша + яйца" }, { name: "Обед", desc: "Индейка + рис" }, { name: "Ужин", desc: "Творог + орехи" }], water: "2.8 л", note: "Следи за углеводами." },
    loss_advanced: { main: "Дефицит 600 ккал. Циклирование углеводов.", foods: ["Курица", "Киноа", "Авокадо", "Протеин"], schedule: [{ name: "Завтрак", desc: "Протеин + овсянка" }, { name: "Обед", desc: "Курица + киноа" }, { name: "Ужин", desc: "Рыба + овощи" }], water: "3 л", note: "Добавь кардио." },
    mass_beginner: { main: "Профицит 400 ккал. 5 приёмов пищи.", foods: ["Рис", "Курица", "Творог 9%", "Орехи"], schedule: [{ name: "Завтрак", desc: "Каша + яйца" }, { name: "Второй завтрак", desc: "Творог" }, { name: "Обед", desc: "Курица + рис" }, { name: "Ужин", desc: "Рыба + гречка" }], water: "2.5 л", note: "Ешь каждые 3 часа." },
    mass_middle: { main: "Профицит 500 ккал. Увеличь углеводы.", foods: ["Говядина", "Макароны", "Бананы", "Протеин"], schedule: [{ name: "Завтрак", desc: "Овсянка + протеин" }, { name: "Обед", desc: "Говядина + макароны" }, { name: "Ужин", desc: "Творог + орехи" }], water: "3 л", note: "Добавь креатин." },
    mass_advanced: { main: "Профицит 600 ккал. Дробное питание.", foods: ["Рис", "Лосось", "Яйца", "Орехи"], schedule: [{ name: "Завтрак", desc: "6 яиц + каша" }, { name: "Обед", desc: "Лосось + рис" }, { name: "Ужин", desc: "Творог + орехи" }], water: "3.5 л", note: "Спи 8 часов." },
    keep_beginner: { main: "Поддержание веса. Сбалансированное питание.", foods: ["Курица", "Овощи", "Гречка", "Яйца"], schedule: [{ name: "Завтрак", desc: "Омлет" }, { name: "Обед", desc: "Курица + гречка" }, { name: "Ужин", desc: "Творог" }], water: "2 л", note: "Контролируй порции." },
    keep_middle: { main: "Поддержание. Баланс БЖУ.", foods: ["Индейка", "Рис", "Овощи", "Орехи"], schedule: [{ name: "Завтрак", desc: "Каша + яйца" }, { name: "Обед", desc: "Индейка + рис" }, { name: "Ужин", desc: "Рыба + овощи" }], water: "2.5 л", note: "Не забывай про жиры." },
    keep_advanced: { main: "Гибкое питание (IIFYM).", foods: ["Любые цельные продукты"], schedule: [{ name: "Завтрак", desc: "Овсянка" }, { name: "Обед", desc: "Мясо + гарнир" }, { name: "Ужин", desc: "Творог" }], water: "2.5 л", note: "Учитывай макросы." }
  }
  const plan = plans[key] || plans.loss_beginner
  nutritionAdvice.value = plan
  // Макросы упрощённые (можно заменить на расчётные)
  macros.value = { calories: 2000, protein: 150, carbs: 200, fats: 60 }
}

function addSuggestedWeights(program, user) {
  // Здесь можно добавить логику подбора весов, но для простоты оставляем как есть
  return program
}

function generatePersonalTips(user) {
  const tips = []
  if (user.goal === 'loss' && user.weight > 90) tips.push('При высоком весе избегай прыжков, замени бег на ходьбу.')
  if (user.goal === 'mass' && user.weight < 65) tips.push('Для набора массы ешь каждые 3 часа, добавь протеин.')
  if (user.level === 'beginner') tips.push('Новичок – сначала отработай технику с маленьким весом.')
  if (user.level === 'advanced') tips.push('Продвинутый – используй периодизацию нагрузки.')
  if (user.gender === 'female' && user.goal === 'mass') tips.push('Девушкам – больше внимания на ягодицы и ноги.')
  personalTips.value = tips.join(' ') || 'Тренируйся регулярно, слушай тело и прогрессируй плавно.'
}
...
function selectProgram() {
  ...
  if (program) {
    ...
    currentProgram.value = addSuggestedWeights(cloned, userData.value)
...
function resetAndGoToSurvey() { router.push('/survey') } // изменено: не удаляет пользователя
...
<style scoped>
/* ----- ОСНОВНЫЕ ПЕРЕМЕННЫЕ (уже есть в вашем файле, оставляем без изменений) ----- */
:root {
  --bg-dark: #06080F;
...
.page { background: #06080F; padding: 100px 80px 80px; font-family: 'Inter', sans-serif; position: relative; }
.glow { background: radial-gradient(ellipse 40% 50% at 80% 20%, rgba(25,70,150,0.2) 0%, transparent 65%); }
.box { max-width: 1200px; margin: 0 auto; animation: fadeUp 0.7s ease both; position: relative; z-index: 1; }
@keyframes fadeUp { from { opacity:0; transform:translateY(20px); } to { opacity:1; transform:translateY(0); } }
.page-label { font-size:11px; letter-spacing:0.3em; text-transform:uppercase; color:rgba(255,255,255,0.25); margin-bottom:8px; }
.page-title { font-family:'Bebas Neue',sans-serif; font-size:48px; color:#FFFFFF; letter-spacing:0.05em; line-height:1; margin-bottom:48px; border-bottom:1px solid var(--border); padding-bottom:20px; }
