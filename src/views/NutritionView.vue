...
export default {
  ...
  methods: {
    ...
    calculateTDEE() {
      ...
      this.targetProtein = Math.round(this.userData.weight * 2)
      this.targetFat = Math.round(this.userData.weight * 0.9)
      
      let carbsFromCal = (this.dailyNorm - (this.targetProtein * 4) - (this.targetFat * 9)) / 4
      this.targetCarbs = Math.max(Math.round(carbsFromCal), 80)
    },
    calculateAndSave() {
      this.calculateTDEE()
      this.saveUserData()
